...
const userBehaviorLimitMiddleware = require('./middleware/userBehaviorLimitMiddleware'); // 引入用户行为限流中间件
...
let chatnioAnonLimiters = [];     // 匿名用户独立限流器
let chatnioAnonEnabled = true;    // 匿名独立限速开关
...
setInterval(() => {
  ...
  recentContentCache.forEach((v, k) => { if (v.firstSeen < expire) recentContentCache.delete(k); });
}, 5 * 60 * 1000);
// 在文件开头引入 dotenv
...
// 内部接口：刷新配置规则缓存（供统计服务在配置变更后调用以立即生效）
app.get('/internal/cache/refresh-config', async (_req, res) => {
  try {
...
// 创建代理中间件
// 🆕 对流式请求注入 stream_options，使上游返回 token 用量（用于 token 统计）
// 仅适用于 OpenAI 兼容协议；Gemini 系列无需处理（usageMetadata 默认携带）
function injectStreamOptions(req, _res, next) {
  if (req.method === 'POST' && req.body?.stream === true && !req.body.stream_options) {
...
app.use('/freelyai', injectStreamOptions, contentModerationMiddleware, freelyaiProxy);
...
// 加载匿名用户独立限速配置
async function loadChatnioAnonLimiter() {
  try {
    const [rows] = await pool.query(
      "SELECT config_value FROM system_configs WHERE config_type='RATE_LIMIT' AND config_key='chatnio_anon_default' AND is_active=1 LIMIT 1"
    );
    if (!rows.length) return;
    const cfg = typeof rows[0].config_value === 'string' ? JSON.parse(rows[0].config_value) : rows[0].config_value;
    chatnioAnonEnabled = cfg.enabled !== false;
    chatnioAnonLimiters = (cfg.enabled !== false && cfg.limits)
      ? cfg.limits.map(({ windowMs, max }) => rateLimit({
          windowMs, max,
          keyGenerator: (req) => {
            const ip = req.body.user_ip || req.headers['x-user-ip'] || req.ip;
            return `chatnio-anon-${ip}`;
          },
          handler: (req, res) => {
            const d = moment.duration(windowMs);
            const fmt = [d.hours() > 0 ? `${d.hours()}小时` : '', d.minutes() > 0 ? `${d.minutes()}分钟` : ''].filter(Boolean).join(' ');
            console.log(`${moment().format('YYYY-MM-DD HH:mm:ss')} [ChatNio Anon Limit] ${req.ip} 匿名用户触发独立限流: ${max}次/${fmt}`);
            return res.status(429).json({ error: { message: `匿名请求过于频繁，请登录后使用或在 ${fmt} 后重试。`, type: 'rate_limit_exceeded', code: '4296' } });
          }
        }))
      : [];
    console.log(`[ChatNio Anon Limiter] 已加载，enabled=${chatnioAnonEnabled}，limiters=${chatnioAnonLimiters.length}`);
  } catch (e) {
    console.error('[ChatNio Anon Limiter] 加载失败:', e.message);
  }
}
loadChatnioAnonLimiter();

// ---- 全局 IP 每日限额中间件 ----
let ipDailyLimitConfig = { limit: 500, enabled: true };
(async () => {
  try {
    const [rows] = await pool.query(
      "SELECT config_value FROM system_configs WHERE config_type='RATE_LIMIT' AND config_key='chatnio_ip_daily_limit' AND is_active=1 LIMIT 1"
    );
    if (rows.length) {
      const cfg = typeof rows[0].config_value === 'string' ? JSON.parse(rows[0].config_value) : rows[0].config_value;
      ipDailyLimitConfig = { limit: cfg.limit || 500, enabled: cfg.enabled !== false };
      console.log(`[ChatNio IP Daily Limit] 已加载，limit=${ipDailyLimitConfig.limit}，enabled=${ipDailyLimitConfig.enabled}`);
    }
  } catch (e) { console.error('[ChatNio IP Daily Limit] 加载失败:', e.message); }
})();

function globalIpDailyLimitMiddleware(req, res, next) {
  if (!ipDailyLimitConfig.enabled) return next();
  const ip = req.body.user_ip || req.headers['x-user-ip'] || req.ip || 'unknown';
  const dateKey = moment().format('YYYY-MM-DD');
  const key = `${dateKey}:${ip}`;
  ipDailyRequestCounts[key] = (ipDailyRequestCounts[key] || 0) + 1;
  if (ipDailyRequestCounts[key] > ipDailyLimitConfig.limit) {
    console.log(`${moment().format('YYYY-MM-DD HH:mm:ss')} [ChatNio IP Daily] ${ip} 超出每日 ${ipDailyLimitConfig.limit} 次限额`);
    return res.status(429).json({ error: { message: `该 IP 今日请求次数已达上限（${ipDailyLimitConfig.limit} 次），请明天再试。`, type: 'daily_ip_limit_exceeded', code: '4297' } });
  }
  next();
}

// ---- max_tokens 上限兜底中间件 ----
let maxTokensConfig = { limit: 8192, enabled: true };
(async () => {
  try {
    const [rows] = await pool.query(
      "SELECT config_value FROM system_configs WHERE config_type='RATE_LIMIT' AND config_key='chatnio_max_tokens' AND is_active=1 LIMIT 1"
    );
    if (rows.length) {
      const cfg = typeof rows[0].config_value === 'string' ? JSON.parse(rows[0].config_value) : rows[0].config_value;
      maxTokensConfig = { limit: cfg.limit || 8192, enabled: cfg.enabled !== false };
      console.log(`[ChatNio Max Tokens] 已加载，limit=${maxTokensConfig.limit}，enabled=${maxTokensConfig.enabled}`);
    }
  } catch (e) { console.error('[ChatNio Max Tokens] 加载失败:', e.message); }
})();

function maxTokensCapMiddleware(req, res, next) {
  if (!maxTokensConfig.enabled) return next();
  if (req.body && req.body.max_tokens && req.body.max_tokens > maxTokensConfig.limit) {
    console.log(`${moment().format('YYYY-MM-DD HH:mm:ss')} [ChatNio Max Tokens] 截断 max_tokens: ${req.body.max_tokens} -> ${maxTokensConfig.limit}`);
    req.body.max_tokens = maxTokensConfig.limit;
  }
  next();
}

// ---- 重复内容检测中间件（1分钟内相同内容最多3次）----
const DUPLICATE_WINDOW_MS = 60 * 1000;
const DUPLICATE_MAX_COUNT = 3;
...
function duplicateContentDetectionMiddleware(req, res, next) {
  ...
  const now = Date.now();
  const entry = recentContentCache.get(key);
  if (entry && (now - entry.firstSeen) < DUPLICATE_WINDOW_MS) {
    entry.count++;
    if (entry.count > DUPLICATE_MAX_COUNT) {
      console.log(`${moment().format('YYYY-MM-DD HH:mm:ss')} [ChatNio Duplicate] ${userId}/${ip} 重复内容第 ${entry.count} 次，已拦截`);
      return res.status(429).json({ error: { message: '检测到重复内容，请勿频繁发送相同请求。', type: 'duplicate_content_detected', code: '4298' } });
    }
  } else {
...
app.use(userBehaviorLimitMiddleware);   // 用户行为限流中间件（小时/日调用次数限制）
...
app.use('/cloudflare', injectStreamOptions, contentModerationMiddleware, cloudflareProxy);

// 应用 /siliconflow 代理中间件，支持文生图、图生图等功能
// 对 SiliconFlow 路由：先审核再转发
app.use('/siliconflow', injectStreamOptions, contentModerationMiddleware, siliconflowProxy);
...
app.use('/freeopenai', injectStreamOptions, contentModerationMiddleware, freeOpenAIProxy);
...
app.use('/chatnio', (req, res, next) => {
  ...
  if (rateLimitersToApply.length>0) {
    ...
    const defaultLimiters = isAnonymous && chatnioAnonEnabled && chatnioAnonLimiters.length > 0
      ? chatnioAnonLimiters
      : (chatnioDefaultEnabled && chatnioDefaultLimiters.length > 0 ? chatnioDefaultLimiters : []);
    if (defaultLimiters.length > 0) {
      console.log(`${moment().format('YYYY-MM-DD HH:mm:ss')} [ChatNio] 模型 ${modelName} 无专属规则，应用${isAnonymous ? '匿名' : '兜底'}限流`);
      (async () => {
        try {
          await Promise.all(defaultLimiters.map(limiter =>
...
}, injectStreamOptions, contentModerationMiddleware, openAIProxy);
