...
// 获取兜底限流配置
router.get('/stats/chatnio-default-limit', async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT id, config_value FROM system_configs WHERE config_type='RATE_LIMIT' AND config_key='chatnio_default' AND is_active=1 LIMIT 1"
    );
    if (!rows.length) {
      return res.json({ data: { enabled: true, limits: [{ max: 300, windowMs: 18000000 }] }, message: '使用默认值（数据库无记录）' });
    }
    ...
    console.error('获取 chatnio 兜底限流配置失败:', e);
    res.status(500).json({ error: '服务器内部错误' });
  }
});

// 更新兜底限流配置（enabled 开关）
router.put('/stats/chatnio-default-limit', async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ error: '兜底限流配置不存在，请先初始化' });
    ...
    console.error('更新 chatnio 兜底限流配置失败:', e);
    res.status(500).json({ error: '服务器内部错误' });
  }
});
