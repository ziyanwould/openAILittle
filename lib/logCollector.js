...
async function getLogs(options = {}) {
  ...
  // 获取本地日志
  if (sources.includes('local') || sources.length === 0) {
    const lines = await readRecentLogLines(requestedLimit * 5);

    for (let i = lines.length - 1; i >= 0; i--) {
      if (allLogs.length >= requestedLimit) break;
      const line = lines[i];
      try {
  ...
  // 获取中间件日志
  if (sources.includes('middleware') || sources.includes('image-middleware')) {
    try {
      const middlewareUrl = process.env.IMAGE_MIDDLEWARE_TARGET;
      if (middlewareUrl) {
        const response = await fetch(`${middlewareUrl}/logs?limit=${Math.min(requestedLimit, 100)}`, {
          timeout: 3000
        });

        if (response.ok) {
          const data = await response.json();
          const middlewareLogs = (data.data || []).map(log => ({
            ...log,
            source: log.source || 'image-middleware'
          }));

          allLogs.push(...middlewareLogs);
        }
      }
    } catch (error) {
      console.warn('[LogCollector] 获取中间件日志失败:', error.message);
    }
  }
  ...
  // 应用时间过滤
  if (sinceTimestamp) {
    allLogs = allLogs.filter(log => Date.parse(log.timestamp) >= sinceTimestamp);
  }

  // 应用级别过滤
  if (targetLevel) {
    allLogs = allLogs.filter(log => String(log.level).toLowerCase() === targetLevel);
  }

  // 限制数量并返回最新的日志
  return allLogs.slice(-requestedLimit);
}
